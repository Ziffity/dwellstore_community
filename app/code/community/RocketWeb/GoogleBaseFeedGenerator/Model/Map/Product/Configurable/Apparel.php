...
class RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Configurable_Apparel extends RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Simple_Apparel {
    ...
    public function map() {
        ...
        foreach ($this->_assocs as $assoc) {
            ...
            if (!$this->getConfigVar('add_out_of_stock_configurable_assoc') && !$assoc->getData('quantity')) {
                if ($this->getConfigVar('log_skip')) {
                    $this->log(sprintf("product id %d sku %s, configurable apparel, skipped - out of stock", $assocId, $assocSku));
                }
                unset($assoc);
                continue;
            }
            ...
            if (!($this->setCacheAssociatedPricesByProduct($assoc) === true)) {
                if ($this->getConfigVar('log_skip')) {
                    $this->log(sprintf("product id %d sku %s, configurable apparel, skipped - could not set price cache", $assocId, $assocSku));
                }
                unset($assoc);
                continue;
            }
            ...
            if ($assocMap->checkSkipSubmission()->isSkip()) {
                if ($this->getConfigVar('log_skip')) {
                    $this->log(sprintf("product id %d product sku %s, configurable apparel, skipped - product has 'Skip from Being Submitted' = 'Yes'.", $assocId, $assocSku));
                }
                continue;
            }
        ...
        // DEBUG
        if (count($assocIds) != count($rows)) {
            $this->log(sprintf("product id %d sku %s, configurable apparel, Less resulting rows than items", $this->getProduct()->getId(), $this->getProduct()->getSku()));
        }
    ...
    protected function formUsConfigurableNonVariant($rows) {
    	...
    	$minimal_price = PHP_INT_MAX;
    	$keep = false;
    	if ((isset($fields[$this->_color_column_name]) && $fields[$this->_color_column_name] == "") || (array_search("size", $must_have) !== false && isset($fields['size']) && $fields['size'] == "")) {
