...
class RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Grouped extends RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Abstract {
    ...
    public function _beforeMap() {
		...
		$this->_assocs = array();
		foreach ($this->getAssociatedProducts() as $assoc) {
			...
			if ($assoc->getStatus() == Mage_Catalog_Model_Product_Status::STATUS_ENABLED) {
                $stockItem = $assoc->getStockItem();
                ...
                $this->_assocs[$assoc->getEntityId()] = $assoc;

				if (!$this->getConfigVar('add_out_of_stock_grouped_assoc') && ($stockItem->getItemId() && !$stockItem->getIsInStock())) {
                    unset($this->_assocs[$assoc->getEntityId()]);
                    if ($this->getConfigVar('log_skip')) {
                        $this->log(sprintf('product id %s, sku %s - skipped out of stock', $assoc->getEntityId(), $assoc->getSku()));
                    }
                }
    ...
    /**
     * Array with associated products.
     *
     * @return array
     */
    public function getAssociatedProducts() {
        ...
        if (is_null($this->_assoc_ids)) {
            $collection = $this->getProduct()->getTypeInstance()->getAssociatedProductCollection($this->getProduct());
            $collection->addAttributeToSelect('*')
                ->addFilterByRequiredOptions()
                ->setPositionOrder()
                ->addStoreFilter(Mage::app()->getStore($this->getStoreId()))
                ->addAttributeToFilter('status', array('in' => array(Mage_Catalog_Model_Product_Status::STATUS_ENABLED)));

            foreach ($collection as $item) {
                $this->_assocs[$item->getEntityId()] = $item;
            }
        }
        return $this->_assocs;
    }
