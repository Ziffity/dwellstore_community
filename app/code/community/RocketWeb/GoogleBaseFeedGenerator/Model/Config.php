...
class RocketWeb_GoogleBaseFeedGenerator_Model_Config extends Mage_Core_Model_Config_Data {
    ...
    public function getAllCategories() {
        ...
        if (is_null($this->_cache_categories)) {
            $collection = Mage::getModel('catalog/category')->getCollection()
                ...
                ->load();
            $this->_cache_categories = $collection->toArray();
        }
   ...
   /**
    ...
    public function getProductCategoryTree(Mage_Catalog_Model_Product $product, $params = array())
    {
        ...
        $categories = array();
        // Store root category 
        //$store_root_cat_id = Mage::app()->getStore($product->getStoreId())->getRootCategoryId();
        // Map
        $map = $params['map'];
        // default value
        ...
        {
            $path_count = count(explode('/', $arr_value['path']));
        ...
        $path = $item['path'];
        // Explode its' items
        $pItemsPieces = explode('/', $path);
        // Default removes
        $default_removes = array(0 => 'default', 1 => 'root');
        // Loop through each items of the path
        foreach($pItemsPieces AS $p)
        {
            // Get the name of the category
            $category = Mage::getSingleton('catalog/category')->load($p);
            // If the static value is set
            if($default_value)
            {
                // Get the comma-separated terms
                $removes = explode(",", $default_value);
                // And combine them with the predefined values
                foreach($default_removes AS $r)
                {
                    array_push($removes, $r);
                }
            } else {
                // Otherwise, only get the defsult values
                $removes = $default_removes;
            }
            // Loop through each item
            foreach($removes AS $key => $value)
            {
                // If the word is not found in categorie's name
                if(!strstr(trim(strtolower($category->getName())), strtolower(trim($value))))
                {
                    // if the category is not already in the array
                    if(!in_array(trim($category->getName()), $categories) && trim($value) !== '')
                    {
                         // Push it to the array 
                        array_push($categories, trim($category->getName()));
                    }
                } else {
                    // Also search the array
                    $index = array_search(trim($category->getName()), $categories);
                    // If found
                    if(count($categories) > 0 && isset($index) && $index >= 0 && strstr(trim(strtolower($categories[$index])), trim(strtolower($value))))
                    {
                        // Unset it
