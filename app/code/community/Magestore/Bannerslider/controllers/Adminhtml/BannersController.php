 ...
 */
class Magestore_Bannerslider_Adminhtml_BannersController extends Mage_Adminhtml_Controller_Action {
    ...
    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('bannerslider/banners')
    ...
    public function editAction() {
        ...
        $model = Mage::getModel('bannerslider/banners')->setStoreId($store)->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('bannerslider/adminhtml_banners_edit'))
                    ->_addLeft($this->getLayout()->createBlock('bannerslider/adminhtml_banners_edit_tabs'));
    ...
    public function saveAction() {        
        if ($data = $this->getRequest()->getPost()) {          
            ...
            $store = $this->getRequest()->getParam('store');
            $model = Mage::getModel('bannerslider/banners');
    ...
    public function massDeleteAction() {
        $bannersliderIds = $this->getRequest()->getParam('banners');
        if (!is_array($bannersliderIds)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($bannersliderIds as $bannersliderId) {
                    $bannerslider = Mage::getModel('bannerslider/banners')->load($bannersliderId);
                    $bannerslider->delete();
                }
        ...
        if (!is_array($bannerIds)) {
            ...
            try {
                foreach ($bannerIds as $bannerId) {
                    $banner = Mage::getSingleton('bannerslider/banners')
                            ->load($bannerId)
    ...
    public function exportCsvAction() {
        ...
        $content = $this->getLayout()->createBlock('bannerslider/adminhtml_banners_grid')->getCsv();
    ...
    public function exportXmlAction() {
        ...
        $content = $this->getLayout()->createBlock('bannerslider/adminhtml_banners_grid')->getXml();
        $this->_prepareDownloadResponse($fileName, $content);
    }

    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('banners');
    }
