...
class Magestore_Onestepcheckout_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function saveAddressOnestepcheckoutAction() {
        ...
        if (isset($shipping_data['email'])) {
            $shipping_data['email'] = trim($shipping_data['email']);
        }
        ...
        if (Mage::helper('onestepcheckout')->isShowShippingAddress()) {
            if (!isset($shipping_data['use_for_billing']) || $shipping_data['use_for_billing'] != '1') {
				...
				$value=$this->getOnepage()->saveBilling($shipping_data, $shipping_address_id);
				 
				}
        }else  $this->getOnepage()->saveBilling($shipping_data, $shipping_address_id);
        $this->getOnepage()->saveShipping($shipping_data, $shipping_address_id);
        if(!$shipping_address_id || $shipping_address_id == '' || $shipping_address_id == null){
            if ($shipping_data['country_id']) {
                Mage::getModel('checkout/session')->getQuote()->getBillingAddress()->setData('country_id', $shipping_data['country_id'])->save();
            }        
        }
        ...
        $use_reward_points = $this->getRequest()->getPost('use_reward_points', false);
        $use_customer_balance = $this->getRequest()->getPost('use_customer_balance', false);
        if ($use_customer_balance == 1) {
            $use_customer_balance = false;
            Mage::getModel('checkout/session')->getQuote()->setUseCustomerBalance(false)->collectTotals()->save();
        } else if ($use_customer_balance == 2) {
            $use_customer_balance = 1;
            Mage::getModel('checkout/session')->getQuote()->setUseCustomerBalance(true)->collectTotals()->save();
        }
        if ($use_reward_points == 1) {
            $use_reward_points = false;
            Mage::getModel('checkout/session')->getQuote()->setUseRewardPoints(false)->collectTotals()->save();
        } else if ($use_reward_points == 2) {
            $use_reward_points = 1;
            Mage::getModel('checkout/session')->getQuote()->setUseRewardPoints(true)->collectTotals()->save();
        }
    ...
    public function save_shippingAction() {
        ...
        try {
            ...
            if ($payment['use_customer_balance'] == 1) {
                $payment['use_customer_balance'] = false;
                Mage::getModel('checkout/session')->getQuote()->setUseCustomerBalance(false)->collectTotals()->save();
            } else if ($payment['use_customer_balance'] == 2) {
                $payment['use_customer_balance'] = 1;
                Mage::getModel('checkout/session')->getQuote()->setUseCustomerBalance(true)->collectTotals()->save();
            }

            if ($payment['use_reward_points'] == 1) {
                $payment['use_reward_points'] = false;
                Mage::getModel('checkout/session')->getQuote()->setUseRewardPoints(false)->collectTotals()->save();
            } else if ($payment['use_reward_points'] == 2) {
                $payment['use_reward_points'] = 1;
                Mage::getModel('checkout/session')->getQuote()->setUseRewardPoints(true)->collectTotals()->save();
            }
    ...
    public function saveOrderAction() {
        ...
        $this->setIgnoreValidation();
        $billing_data['email'] = $shipping_data['email'];
        ...
        //resave billing address to make sure there is no error if customer change something in billing section before finishing order
		 if (!isset($shipping_data['use_for_billing']) || $shipping_data['use_for_billing'] != '1') {
			...
			$result = $this->getOnepage()->saveBilling($shipping_data, $shipping_address_id);
